...
import json
from .models import Venta, VentaItem, Compra, CompraItem, TipoComprobante, ClienteProveedor
...
from .forms import VentaForm, CompraForm
...
class VentasTrazabilidadCargaView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        return render(request, 'facturacion/ventas_trazabilidad_carga.html', {'form': form, 'puntos_venta': puntos_venta})
                ...
                if venta_temp.condic == 1 and venta_temp.tipo.codigo.isdigit():
                    ...
                    try:
                        ...
                        if cbte_tipo_num == 1 and cond_iva_rec not in [1, 6]:
                            ...
                            return render(request, 'facturacion/ventas_trazabilidad_carga.html', {'form': form, 'puntos_venta': PuntoVenta.objects.filter(sucursal_id=request.session.get('sucursal_id'), activo=True)})
...
class ComprasTrazabilidadCargaView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        return render(request, 'facturacion/compras_trazabilidad_carga.html', {
            'form': form,
            'empresa': empresa,
...
def compras_trazabilidad_item_add(request):
    ...
    response = render(request, 'facturacion/partials/compra_trazabilidad_items_tabla.html', {'items': items})
...
def compras_trazabilidad_item_remove(request, index):
    ...
    request.session['compra_trazabilidad_items_temp'] = items
    request.session.modified = True
    return render(request, 'facturacion/partials/compra_trazabilidad_items_tabla.html', {'items': items})
