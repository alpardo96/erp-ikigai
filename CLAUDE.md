...
- **Importes:** siempre formato es-AR centralizado — en pantallas/reportes: filtro `{{ valor|formato_ar }}` (separador de miles obligatorio `1.234.567,89`) y clase `.fInputAR` con `static/js/formato_ar.js` en inputs. En exportaciones (Excel/CSV): valor numérico puro (`float`/`Decimal`) con máscaras de celda en Excel (`#,##0.00`) para no degradar a texto y permitir cálculos. Prohibido formatear a mano.
