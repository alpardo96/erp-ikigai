...
def facturar_comprobante(wsfe, datos_factura):
    ...
    try:
        ...
        # Procesar tributos múltiples
        tributos_lista = []
        total_tributos = 0.0
        
        for i in range(1, 4):
            trib_id = to_int(datos_factura.get(f'trib{i}_id', 0))
            if trib_id > 0:
                importe = to_float(datos_factura.get(f'trib{i}_importe', 0))
                tributos_lista.append({
                    'Id': trib_id,
                    'Desc': str(datos_factura.get(f'trib{i}_desc', '')).strip(),
                    'BaseImp': to_float(datos_factura.get(f'trib{i}_base', 0)),
                    'Alic': to_float(datos_factura.get(f'trib{i}_alic', 0)),
                    'Importe': importe
                })
                total_tributos += importe

        # Calcular totales para cuadrar con AFIP
        imp_neto = to_float(datos_factura.get('imp_neto', 0))
        imp_op_ex = to_float(datos_factura.get('imp_op_ex', 0))
        imp_iva = to_float(datos_factura.get('imp_iva', 0))
        imp_tot_conc = to_float(datos_factura.get('imp_tot_conc', 0))
        imp_total_calculado = imp_neto + imp_op_ex + imp_iva + total_tributos + imp_tot_conc
        ...
        # Asignar bloque de Tributos si corresponde
        if tributos_lista:
            detalle['Tributos'] = {
                'Tributo': tributos_lista
            }
...
if __name__ == "__main__":
    ...
    archivo_entrada_xlsx = os.path.join(RUTA_BASE, "plantilla_facturacion_tributos.xlsx")
    archivo_entrada_csv = os.path.join(RUTA_BASE, "plantilla_facturacion_tributos.csv")
    archivo_usado = None
