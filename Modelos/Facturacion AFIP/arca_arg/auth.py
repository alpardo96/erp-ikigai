...
class ArcaAuth:
    ...
    def create_tra(self, expiration_time):
        ...
        # Le restamos 10 minutos a la hora actual para evitar que AFIP rechace el ticket
        # si nuestro reloj está adelantado unos segundos respecto al de ellos.
        now = datetime.datetime.now(tz_buenos_aires) - datetime.timedelta(minutes=10)
        ...
        <loginTicketRequest version="1.0">
            <header>
                ...
                <expirationTime>{expiration_time.isoformat()}</expirationTime>
            </header>
    ...
    def sign_tra(self, tra_xml):
        ...
        with open(settings.CERT_PATH, 'rb') as cert_file:
    ...
    def request_new_ta(self):
        ...
        wsdl_wsaa = settings.WSDL_WSAA_PROD if settings.PROD else settings.WSDL_WSAA_HOM
        ...
        with open(settings.TA_FILES_PATH + self.service + ".pkl", "wb") as f:
            pickle.dump(LoginTicket(response), f)

    def load_ta(self):
        """Carga el TA almacenado, si es válido."""
        
        if os.path.exists(settings.TA_FILES_PATH + self.service + ".pkl"):
            with open(settings.TA_FILES_PATH + self.service + ".pkl", "rb") as f:
