...
def run():
    print("Iniciando Fase 3: Bloque Tesorería y Flujos (Armería)")
    ...
    # 1. Bancos
    bancos_dbf = os.path.join(dir_balance, 'bancos.dbf')
    bancos_dict = {}
    ...
    # 2. Cuentas Bancarias
    cta_cte_dbf = os.path.join(dir_balance, 'cta_cte.dbf')
    ...
    # 3. Tarjetas
    tarjetas_dbf = os.path.join(dir_balance, 'tarjetas.dbf')
    ...
    enc_caja_dbf = os.path.join(dir_balance, 'enc_caja_diaria.dbf')
    ...
    caja_mov_dbf = os.path.join(dir_balance, 'caja_diaria.dbf')
    if os.path.exists(caja_mov_dbf):
        ...
        for row in table:
            ...
            if not fecha_mov: continue

            condic = int(col(row, 'CONDIC', 'ID_CON', default=1) or 1)
            if condic not in (1, 2): condic = 1
