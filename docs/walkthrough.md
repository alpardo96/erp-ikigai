...
- **Objetivo o Tarea**: Implementaci�n de Extensiones Armer�a, Trazabilidad, e Impresi�n de Preventas
- **Archivos creados o modificados**:
...
- **Detalle T�cnico e implicaciones**:
    - Se agreg� bot�n de edici�n de cliente en modal en las vistas de carga de ventas y preventas.
    - Se reemplazaron alertas de bucle infinito (alert) por notificaciones Swal.fire.
    - Se incorpor� soporte de impresi�n de Remitos de Preventa con \generar_pdf_preventa\.
    - Se a�adi� bot�n PDF en el modal de Trazabilidad (Timeline) y en la bandeja de Autorizaciones para descargar el comprobante en nueva pesta�a.
    - Se ejecut� script de reset de secuencias para arreglar error de ID ya existente y se aplicaron migraciones para los borrados l�gicos.
- **Resultado de las pruebas**: Vistas actualizadas, PDF generando correctamente y bug de alertas JS solucionado.


## Antigravity
- **Fecha/D�a**: 16 de Septiembre de 2026
- **Objetivo o Tarea**: Arreglar selecci�n de cliente en Armer�a y automatizar facturaci�n de reservas SIGIMAC.
- **Archivos creados o modificados**:
...
- **Detalle T�cnico e implicaciones**:
    - Se solucion� el bug donde \clienteVentaSeleccionado\ no llegaba a \document.body\ al seleccionarse desde el typeahead, haciendo que el evento se dispare sobre \document.body\ y burbujee de forma correcta. Esto arregl� la alerta de reserva pendiente "que no estaba funcionando".
    - El bot�n "Facturar" en SIGIMAC ahora a�ade \?cliente_id=X\ a la URL.
    ...
    - El autocompletado de serie \	ypeahead_series_trazabilidad\ ahora intercepta el \cliente_id\ seleccionado en la factura y, de estar el campo de serie vac�o (cuando el usuario hace focus/clic), carga directamente los subproductos (series) que ese cliente tenga en sus reservas PENDIENTES, facilitando seleccionar el arma reservada sin escribir nada.
- **Resultado de las pruebas**: Navegaci�n directa desde bandeja SIGIMAC hasta Trazabilidad con cliente precargado y arma lista para ser seleccionada al hacer clic en el buscador de series.


## Antigravity (Correcci�n)
- **Fecha/D�a**: 16 de Septiembre de 2026
- **Objetivo o Tarea**: Arreglar error de ID vac�o al clickear 'Facturar' y alerta no renderizada.
- **Archivos modificados**:
    - \erticalidades/armeria/templates/armeria/partials/reservas_tabla_parcial.html\ [MODIFY]
    - \erticalidades/armeria/templates/armeria/ventas_trazabilidad_carga.html\ [MODIFY]
- **Detalle T�cnico**:
    - Se cambi� \{{ r.cliente.id }}\ por \{{ r.cliente.pk }}\ en el bot�n Facturar de la tabla de reservas. Esto ocurr�a porque el modelo \ClienteProveedor\ tiene un primary key personalizado (\codigo_id\), lo que provocaba que Django devolviera un string vac�o al usar \.id\ en el template, arruinando la URL generada.
    - Se cambi� el uso de \etch()\ crudo en JS por \htmx.ajax()\ al escuchar el evento de cliente seleccionado. Esto asegura que la petici�n de validaci�n de reserva SIGIMAC viaje con todas las cookies de sesi�n y cabeceras necesarias, evitando que la vista de verificaci�n fallara al no reconocer la sesi�n (\empresa_id\).


    - Se regres� el sistema de alerta al m�todo \etch()\ (el cual funcionaba antes) pero montado sobre el \document.body\, debido a que HTMX por defecto cancela las peticiones as�ncronas concurrentes originadas en el mismo elemento (o si no se les especifica un 'source' expl�cito). Al volver a usar la API nativa de JavaScript, permitimos que la carga de los detalles fiscales del cliente y la alerta de reserva se procesen en paralelo sin chocarse ni cancelarse.
    - **Protecci�n de Concurrencia (Doble Venta)**: Se a�adi� una validaci�n temprana en \VentasTrazabilidadCargaView\ al momento de hacer un POST. Si se env�a un \
eserva_id\, se verifica en milisegundos que siga en estado 'PENDIENTE' antes de tocar AFIP. Si otra PC ya la factur� mientras esta ten�a la pesta�a abierta, el sistema bloquear� la venta y devolver� un mensaje de error amigable, previniendo as� un doble cobro y doble baja de stock.


## [16 de Septiembre de 2026] - Notas de Reservas SIGIMAC
**Objetivo:** Permitir al vendedor a�adir una nota interna al confirmar la preventa de un arma (producto trazable), y visualizarla en la bandeja del cajero de Reservas SIGIMAC.
**Archivos Modificados:**
- \acturacion/models.py\: A�adido \
otas_sigimac\ a \Preventa\.
- \erticalidades/armeria/models.py\: A�adido \
otas\ a \ReservaArma\.
...
- \	emplates/facturacion/preventa_carga.html\: Interceptado el bot�n de Confirmar Preventa para levantar el modal \Swal.fire\ solicitando la nota.
...
- \erticalidades/armeria/templates/armeria/partials/reservas_tabla_parcial.html\: A�adida columna Notas y bot�n interactivo para visualizar el contenido con \Swal.fire\.
**Resultado:** Al facturar un arma, salta el modal de notas y las notas son copiadas a la bandeja de Armer�a, reemplazando el flujo f�sico de anotaciones en papel.


- **Ajustes Adicionales (16 Sep)**: 
  - Se configur� \llowOutsideClick: false\ en el modal de notas para evitar cierres accidentales por miss-clicks.
  - Se a�adieron expl�citamente los botones de Aceptar (Confirmar) y Cancelar (Cerrar).
  - Se implement� validaci�n de campo obligatorio dentro de \preConfirm\: no se permite avanzar si la nota est� vac�a, arrojando un mensaje de error visual dentro del modal de SweetAlert.


  - Se forz� el color del texto de los botones del modal (\	ext-white !font-bold\) a trav�s de \customClass\ para evitar que TailwindCSS sobreescriba los colores por defecto de SweetAlert2, corrigiendo el error visual de que los botones de Aceptar/Cancelar se vieran oscuros o invisibles.


  - Se corrigi� la visibilidad del bot�n en la columna de Notas de la bandeja de Reservas. El �cono previo (\a-note-sticky\) no estaba siendo renderizado por la versi�n de FontAwesome en uso, lo que causaba que el bot�n tuviera dimensiones 0x0 y la celda apareciera vac�a. Se reemplaz� por \a-comment-dots\ y se le dio estilo estructurado (padding, fondo y borde) para asegurar su correcta renderizaci�n e interactividad.


  - Se cambi� el �cono del bot�n de Notas en la tabla de Reservas por un SVG nativo embebido, ya que hubo problemas de compatibilidad con la versi�n de FontAwesome local que imped�an la correcta renderizaci�n de cualquier �cono en esa vista, mostrando un bot�n vac�o.
...
- **Objetivo o Tarea**: Implementación completa del flujo de Notas SIGIMAC en Preventa/Reservas y estabilización UI de Trazabilidad.
- **Archivos creados o modificados**:
    ...
    - erticalidades/armeria/views.py [MODIFY]
- **Detalle Técnico e implicaciones**:
    - **Pase y Detección de Reservas en Trazabilidad**: El botón 'Facturar' en Reservas ahora vincula mediante ?cliente_id=X utilizando .pk (soportando primary key codigo_id), cargando automáticamente datos fiscales y sugiriendo en el buscador de series las armas reservadas activas del cliente.
    - **Protección de Concurrencia**: Verificación de estado 'PENDIENTE' de la reserva antes de enviar a AFIP/guardar, previniendo dobles ventas accidentales entre terminales simultáneas.
    - **Notas SIGIMAC en Preventa**: Intercepción de 'Confirmar Preventa' si hay productos trazables para ingresar notas obligatorias dirigidas a caja/armería (llowOutsideClick: false). Las notas se guardan y se transfieren a la ReservaArma en la cobranza de seña/preventa.
...
**Archivos modificados:**
- core/templatetags/formato_tags.py [MODIFY]: Se optimizó el filtro formato_ar para formatear con precisión y sustitución directa de separadores a partir de cadenas float/Decimal, soportando valores nulos, vacíos y ceros.
- templates/facturacion/ventas_listado.html [MODIFY]: Se cargó formato_tags y se aplicó |formato_ar en Neto, IVA, Total y la fila de Totales del tfoot.
- templates/facturacion/compras_listado.html [MODIFY]: Se cargó formato_tags y se aplicó |formato_ar en Neto, IVA, Total y la fila de Totales del tfoot.
- docs/planes/091_separador_miles_reportes.md [NEW]: Plan de implementación consensuado en sesión Grill-Me.

**Detalles Técnicos:**
- No se modificaron settings globales ni parsers de backend para asegurar que ningún proceso de captura (OCR, Excel) o formularios HTMX (.fInputAR) sufran efectos colaterales.
- Las exportaciones a Excel (.xlsx) conservan sus valores numéricos originales (float/Decimal) con máscaras de celda (#,##0.00) para preservar la capacidad de cálculo en hojas de cálculo.
...
**Objetivo:** Correcci�n de referencias Javascript faltantes (fMiles / parseAR) por la centralizaci�n de formato_ar.
**Archivos modificados:**
- erticalidades/armeria/templates/armeria/ventas_trazabilidad_carga.html`n- 	emplates/facturacion/ventas_trazabilidad_carga.html`n- 	emplates/facturacion/ventas_carga.html`n- 	emplates/facturacion/preventa_carga.html`n- 	emplates/facturacion/compras_carga.html`n- 	emplates/tesoreria/caja_mostrador_abrir.html`n**Detalle T�cnico:** Se reemplazaron todas las llamadas a las funciones viejas Miles y parseAR que quedaron hu�rfanas en los event listeners tras el �ltimo commit. Ahora todas llaman a las nuevas funciones centralizadas ormatearAR y desformatearAR exportadas globalmente desde core/static/js/formato_ar.js.
